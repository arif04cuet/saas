    ...
    $.fn.formbuilder = function (options) {
        ...
        return this.each(function () {
            ...
            var controlBox = function (target) {
                ...
                save_button = '<input type="submit" id="' + save_id + '" style="float:right;" class="frmb-submit btn btn-success" value="' + opts.messages.save + '"/>';
                ...
                ///////////////////////////////////////////////////////////
                //========================================================//
//                var extra_hidden_feilds = '';
//                extra_hidden_feilds = '<input type="hidden" id="hide_name_bn" value=""/><input type="hidden" id="hide_name_en" value=""/><input type="hidden" id="hide_active" value="0"/>';
//                $(ul_obj).after(extra_hidden_feilds);
                //========================================================//
                ////////////////////////////////////////////////////////////
            ...
            var appendFieldLi = function (title, field_html, required, help) {
                ...
                li += '<input class="required" type="checkbox" value="1" name="required-' + last_id + '" id="required-' + last_id + '"' + ((required == 'true') ? ' checked="checked"' : '') + ' /></div>';
                li += field;
                li += '</div>';
                li += '</div>';
                li += '</li>';
            ...
            var save = function () {
                if (opts.save_url) {
                    ...
                    var dt = $(ul_obj).serializeFormList({
                        prepend: opts.serialize_prefix
                    }); //+ "&form_id=" + form_db_id;

                    dt += "&machine_name=" + $("#machine_name").val();
                    dt += "&active=" + $("#active").val();
                    dt += "&form_title=" + $("#form_title").val();
                    dt += "&action_type=" + $("#action_type").val();
                    dt += "&action_path=" + $("#action_path").val();
                    dt += "&form_msg=" + $("#form_msg").val();
                    dt += "&form_email=" + $("#form_email").val();
                    dt += "&header_bn=" + $("#header_bn").val();
                    dt += "&header_en=" + $("#header_en").val();
                    dt += "&footer_bn=" + $("#footer_bn").val();
                    dt += "&footer_en=" + $("#footer_en").val();
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            //alert(result);
                            //window.location.replace("http://cabinet.portal.gov.bd/npfadmin/webforms/search");
                            window.location.href = '/npfadmin/webforms/search';
