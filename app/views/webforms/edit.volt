{{ content() }}
<form method="post" autocomplete="off" action="<?php echo $this->url->get('webforms/updateform') ?>">
<ul class="pager">
...
<script>
    $(function () {
        var dt = JSON.parse('{{ flds }}');
        //console.log(dt);
        var $t = $('#my-form-builder').formbuilder({
            'save_url': '<?php echo $this->url->get("webforms/updateform") ?>',
            'load_url': false,
            'dt': dt,
            'useJson': true
        });
